...
class MenuService:
    ...
    def add_coffee(self, name, price, size, description, is_available=True):
        db = Database()
        return db.execute(
            """INSERT INTO menu 
            (name, price, size, description, is_available) 
            VALUES (%s, %s, %s, %s, %s)""",
            (name, price, size, description, is_available)
        )
    ...
    def update_coffee(self, item_id, name, price, size, description, is_available):
        db = Database()
        db.execute(
            """UPDATE menu SET 
            ...
            WHERE item_id=%s""",
            (name, price, size, description, is_available, item_id)
        )
